...
`bd_active_follow_up_emails.json`
...
Important:
Some raw-ingested emails do not have `processing_status`.
...
When `search_emails` identifies a relevant thread, use `get_thread` when additional conversation context is necessary.
...
`search_emails` for Miami
→ inspect relevant threads
→ `list_meetings` if relevant
...
- raw-only emails may not have `processing_status`
...
"Call search_emails with this sender and then call get_thread with this ID."
